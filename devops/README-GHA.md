...
2. Go to your repository at [gebara/irontreecms](https://github.com/gebara/irontreecms).
3. Click on `Settings` at the top-right corner.
4. In the left sidebar, click on `Environments`.
5. Press `New environment`.
6. Name the environment `cms.irontree.co.kr` and hit `Configure environment`.
...
|-------------|--------------|-------------|
| DEPLOY_HOST | cms.irontree.co.kr | The hostname or IP address of your Docker Swarm manager. |
...
- [Backend Tests Workflow](https://github.com/gebara/irontreecms/actions/workflows/backend.yml)
- [Frontend Tests Workflow](https://github.com/gebara/irontreecms/actions/workflows/frontend.yml)

Upon successful completion of the tests, Docker images for the Backend (`ghcr.io/gebara/irontreecms-backend`) and Frontend (`ghcr.io/gebara/irontreecms-frontend`) will be available.

### Initiating Manual Deployment

1. Navigate to [Manual Deployment of cms.irontree.co.kr](https://github.com/gebara/irontreecms/actions/workflows/manual-deploy.yml).
...
- Initiate a new deployment using the `devops/stacks/cms.irontree.co.kr.yml` stack.
